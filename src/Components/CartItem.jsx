    ...
    <>
      <div className="flex gap-2 shadow-md rounded-lg p-2">
        <MdDelete
          ...
          onClick={() =>{ dispatch(removeFromCart({ id, img, price, qty }))
          toast(`${name} Removed! `, {
            icon: '👏',
          });
        }}
        ...
        <div className="leading-5">
          ...
          <div className="flex justify-between">
            ...
            <div className="flex justify-center items-center gap-2 absolute right-7">
              ...
              />
              <span>{qty}</span>
              <AiOutlinePlus onClick={()=> dispatch(incrementQty({id}))} className="border-2 border-gray-600 text-gray-600 hover:text-white hover:bg-green-500 hover:border-none rounded-md text-xl p-1 transition-all ease-linear cursor-pointer" />
            </div>
          </div>
        </div>
      </div>
